...
% This function is written by Zhe Liu
    ...
    methods
        ...
        %% Generate random population
            ...
            CH = Archive(floor(end/2)+1:2*floor(end/2)); 
        %% External archive initialization
            while Algorithm.NotTerminated(Archive)
        %% Optimization
                MatingPool_UCP = [randperm(length(CF)),randperm(length(CF))];
                MatingPool_CP = [randperm(length(CH)),randperm(length(CH))];
                Offspring1 = OperatorDE(Problem, CF, CF(MatingPool_UCP(1:end/2)), CF(MatingPool_UCP(end/2+1:end)),{1,0.5,1,20*rand()^t});
                Offspring2 = OperatorDE(Problem, CH, CH(MatingPool_CP(1:end/2)), CH(MatingPool_CP(end/2+1:end)),{1,0.5,1,20});
                CF = PopulationUpdateCDS([CF, Offspring1, Offspring2], floor(Problem.N/2), k, false);  
                CH = PopulationUpdateCDS([CH, Offspring1, Offspring2], floor(Problem.N/2), k, true);
                Archive = ArchiveUpdate([Archive, CH], Problem.N);  
            end
        end
    end
end
